...
const CourseView = () => {
    ...
    const renderLectureItem = (lecture, index) => {
    ...
    const groupedLectures = React.useMemo(() => {
        ...
        const hasExplicitSections = lectures.some(l => l.section && l.section.trim() !== "");

        if (hasExplicitSections) {
            const sectionMap = {};
            lectures.forEach((lecture, idx) => {
                const sec = (lecture.section || "").trim();
                if (sec) {
                    if (!sectionMap[sec]) {
                        const newGroup = { sectionName: sec, id: `sec-${sec}`, isExplicitSection: true, lectures: [] };
                        sectionMap[sec] = newGroup;
            ...
            lectures.forEach((lecture, idx) => {
                ...
                if (isContinuation && currentLegacyGroup) {
                    ...
                    if (currentLegacyGroup) {
                        groups.push(currentLegacyGroup);
                    }
                    let secName = lecture.title || `Section ${groups.length + 1}`;
                    const match = lecture.title?.match(/(.*?)\s*[\(\-]?\s*(part|پارٹ|lecture|لیکچر)/i);
                    if (match && match[1].trim()) {
                        secName = match[1].trim();
                    }
                    currentLegacyGroup = { sectionName: secName, id: `sec-${idx}`, isExplicitSection: false, lectures: [lecture] };
                }
            });
            if (currentLegacyGroup) {
                groups.push(currentLegacyGroup);
    ...
    useEffect(() => {
        if (currentLecture && groupedLectures.length > 0) {
            const activeGroup = groupedLectures.find(g => g.lectures.some(l => l.id === currentLecture.id));
            if (activeGroup && !expandedSections[activeGroup.id]) {
                setExpandedSections(prev => ({ ...prev, [activeGroup.id]: true }));
            }
        }
    }, [currentLecture, groupedLectures]);
    ...
    return (
        <div className="h-screen w-screen flex items-center justify-center">
            ...
            {/* PDF Viewer Modal */}
            {activePdfUrl && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-10">
                    <div className="bg-white w-full h-full max-w-5xl rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 bg-white">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-lg font-bold text-gray-900 leading-tight">Document Resource</h3>
                                    <p className="text-xs text-gray-500 font-medium">Viewing lecture materials</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-3">
                                <a
                                    href={activePdfUrl}
                                    download
                                    className="hidden sm:flex items-center gap-2 px-4 py-2 bg-gray-50 text-gray-600 rounded-xl hover:bg-gray-100 transition-all text-sm font-bold"
                                >
                                ...
                                <button
                                    ...
                                    className="p-2.5 bg-gray-50 text-gray-400 hover:text-gray-600 rounded-xl hover:bg-gray-100 transition-all"
                                >
                                    <X size={24} />
                                </button>
                            </div>
                        ...
                        <div className="flex-1 bg-gray-100 relative">
                            {/* We use an iframe to display the PDF. 
                                For Cloudinary raw resources, adding fl_attachment:false doesn't work well, 
            ...
            {activeAudioUrl && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-md animate-in slide-in-from-bottom-4 duration-300">
                    <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl border border-blue-100 p-4 flex flex-col gap-3">
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2">
                                <div className="w-8 h-8 bg-blue-500 rounded-lg flex items-center justify-center">
                                    <Volume2 size={18} className="text-white animate-pulse" />
                                </div>
                                <span className="text-sm font-bold text-gray-800">Playing Audio Resource</span>
                            </div>
                            <button
